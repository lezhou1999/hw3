...
void TestSubtract(){
	...
	TimeCode tc4 = TimeCode(1, 0, 0);
	assert((tc4==tc1)==true );
	assert((tc2==tc1)==false );
	assert((tc1<tc2)==false);
	TimeCode tcTimes = tc1*2;
	TimeCode tcDivde = tc2/5;

	assert(tcTimes.ToString() == "2:0:0");
	assert(tcDivde.ToString() == "0:10:0");
	

	
	
	
	TimeCode tc6 = TimeCode(1, 15, 45);
	try{
		TimeCode tc5 = tc1 - tc6;
...
void TestSetMinutes()
{
	cout << "Testing SetMinutes1" << endl;

	TimeCode tc = TimeCode(8, 5, 9);
	tc.SetMinutes(15); // test valid change
	assert(tc.ToString() == "8:15:9");

	cout << "Testing SetMinutes2" << endl;
	
	tc.SetMinutes(0); // test valid change
...
void TestSetHours()
{
	...
	TimeCode tc = TimeCode(8, 5, 9);
	tc.SetHours(9); // test valid change
	assert(tc.ToString() == "9:5:9");

	try
	{
		tc.SetHours(80);  // test invalid change
		assert(false);
	}
	catch (const invalid_argument &e)
	{
